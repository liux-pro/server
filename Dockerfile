...
ADD pom.xml .
RUN . "$HOME/.sdkman/bin/sdkman-init.sh" && mvn -ntp -Pnative -Phuawei-repo dependency:resolve

FROM prepare as builder
WORKDIR /app
COPY . .
RUN . "$HOME/.sdkman/bin/sdkman-init.sh" && mvn -ntp -Pnative -Phuawei-repo -DskipTests package
